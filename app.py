...
def single_repos_meta_single_repos_analysis(user, token, repo_name, branch, api=True)->json or dict:
    ...
    if resp_status_code == 200:
        ...
        if  check_lang_exit(user=user, repo=repo_name, headers=headers, lang_list=lang_list):
            ...
            stderr, return_code, additions_dict, files, file_check_results, commit_history_dict, converted_nbs = run_to_get_adds_and_save_content(user=user ,
                                                                                                                                                  repo_name=repo_name, repo_dict=repo_details[0],
                                                                                                                                                  file_ext=[".py", ".ipynb", ".js"], branch=branch, token=token)
